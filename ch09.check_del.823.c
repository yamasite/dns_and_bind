...
**       gcc -o check_del -I../../include -I../../port/linux/include \
...
int log_ctx_valid = 0;
int loading = 0;
options server_options = NULL;
const char panic_msg_no_options[] = "no server_options in NS_OPTION_P";
const int hashsizes[] = { 2, 11, 113, 337, 977, 2053, 4073, 8011, 16001, 99887, 0 };
...
** the hash tables.
*/
db_set_update(char *name, struct databuf *newdp, void **state,
...

{
	...
	*/
	if((newdp != NULL) && (newdp->d_type == T_NS)){
			...
			*/
			if((failure == ECONNREFUSED) && (notrunning < 200))
				string_run[notrunning++] = 
						savestr((const char *)newdp->d_data, 0);
